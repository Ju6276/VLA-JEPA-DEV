...
class VLATrainer(TrainerUtils):
    def __init__(self, cfg, model, vla_train_dataloader, optimizer, lr_scheduler, accelerator):
        ...
        self.completed_steps = int(resume_step) if resume_step is not None else 0
        if self.completed_steps < 0:
            raise ValueError(f"trainer.resume_step must be non-negative, got {self.completed_steps}")
    ...
    def prepare_training(self):
        ...
        if hasattr(self.config.trainer, "pretrained_checkpoint") and self.config.trainer.pretrained_checkpoint:
            ...
            raise ValueError("trainer.resume_step requires trainer.pretrained_checkpoint")

        # Weight-only checkpoints do not contain scheduler state. Align the
        # newly-created scheduler with the resumed global step.
        if self.completed_steps > 0:
            self.lr_scheduler.step(self.completed_steps)
            self.accelerator.print(
                f"Continuing weight-only training from global step {self.completed_steps}"
            )
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        with self.accelerator.accumulate(self.model):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                ...
                # Only backprop the scalar objective. Extra keys (component losses /
                # monitoring metrics) are logged but must not be summed into the loss.
            ...
            result_dict = {
                k: (v.item() if torch.is_tensor(v) else float(v)) for k, v in output_dict.items()
            }
