...
class TrainerUtils:
    @staticmethod
    def _load_compatible_state_dict(module, checkpoint, module_name: str):
        ...
        for key, value in checkpoint.items():
            ...
            if tuple(target_state[key].shape) != tuple(value.shape):
                mismatched_keys.append((key, tuple(value.shape), tuple(target_state[key].shape)))
                continue
        ...
        if rank == 0:
            print(
                f"✅ loaded compatible parameters for '{module_name}': "
                f"loaded={len(compatible_state)}, missing={len(missing_keys)}, "
                f"unexpected={len(unexpected_keys)}, mismatched={len(mismatched_keys)}"
            )
            if missing_keys:
                print(f"   missing sample: {missing_keys[:20]}")
            if unexpected_keys:
                print(f"   unexpected sample: {unexpected_keys[:20]}")
            if mismatched_keys:
                print(f"   mismatched sample: {mismatched_keys[:20]}")
            if load_result.unexpected_keys:
                print(f"   loader unexpected: {load_result.unexpected_keys[:20]}")
            if load_result.missing_keys:
                print(f"   loader missing: {load_result.missing_keys[:20]}")
        ...
        if reload_modules:  # partial load
            ...
            for path in module_paths:
                ...
                try:
                    ...
                    if sub_state_dict:
                        TrainerUtils._load_compatible_state_dict(module, sub_state_dict, path)
            ...
            try:
                TrainerUtils._load_compatible_state_dict(model, checkpoint, "<full_model>")
