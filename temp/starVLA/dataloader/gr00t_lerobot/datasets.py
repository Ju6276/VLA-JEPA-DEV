...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        try:
            if steps_path.exists():
                ...
                return self._filter_complete_endpoint_steps(cached_data["steps"])
            else:
        ...
        all_steps = self._filter_complete_endpoint_steps(all_steps)
    ...
    def _get_steps_config_key(self) -> str:
        ...
        config_dict = {
            ...
            "delta_indices": {
                ...
                for key, value in sorted(self.delta_indices.items())
            },
        }
    ...
    def _endpoint_max_delta(self) -> int:
        """Return the largest future offset needed for complete video/state endpoints."""
        endpoint_max_delta = 0
        for key in self.modality_keys.get("video", []):
            indices = self.delta_indices.get(key)
            if indices is not None and len(indices) > 0:
                endpoint_max_delta = max(endpoint_max_delta, int(np.max(indices)))
        for key in self.modality_keys.get("state", []):
            indices = self.delta_indices.get(key)
            if indices is not None and len(indices) > 0:
                endpoint_max_delta = max(endpoint_max_delta, int(np.max(indices)))
        return endpoint_max_delta

    def _filter_complete_endpoint_steps(self, steps: list[tuple[int, int]]) -> list[tuple[int, int]]:
        """Keep only starts whose endpoint window is fully inside the episode."""
        endpoint_max_delta = self._endpoint_max_delta()
        if endpoint_max_delta <= 0:
            return steps
        ...
        trajectory_lengths = {
            _key(trajectory_id): int(length)
            for trajectory_id, length in zip(self.trajectory_ids.tolist(), self.trajectory_lengths.tolist())
        }
        filtered_steps = [
            (trajectory_id, base_index)
            for trajectory_id, base_index in steps
            if int(base_index) + endpoint_max_delta < trajectory_lengths[_key(trajectory_id)]
        ]
        dropped = len(steps) - len(filtered_steps)
        if dropped:
            print(
                f"Filtered {dropped} endpoint-incomplete steps "
                f"(max endpoint delta={endpoint_max_delta}) from {self.dataset_name}."
            )
...
class LeRobotMixtureDataset(Dataset):
    ...
    def _valid_time_mask(self, dataset: LeRobotSingleDataset, trajectory_id: int, key: str, base_index: int):
        trajectory_index = dataset.get_trajectory_index(trajectory_id)
        max_length = int(dataset.trajectory_lengths[trajectory_index])
        raw_indices = dataset.delta_indices[key] + base_index
        valid_mask = np.logical_and(raw_indices >= 0, raw_indices < max_length)
        clamped_indices = np.clip(raw_indices, 0, max_length - 1)
        return valid_mask.astype(np.bool_), clamped_indices

    def _endpoint_valid_mask(self, valid_mask: np.ndarray, clamped_indices: np.ndarray) -> np.bool_:
        if valid_mask.size < 2 or clamped_indices.size < 2:
            return np.bool_(False)
        return np.bool_(valid_mask[0] and valid_mask[-1] and clamped_indices[0] != clamped_indices[-1])
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                video_mask, video_indices = self._valid_time_mask(
                    dataset,
                    trajectory_name,
                    dataset.modality_keys["video"][0],
                    step,
                )
                action_mask, _ = self._valid_time_mask(
                    dataset,
                    trajectory_name,
                    dataset.modality_keys["action"][0],
                    step,
                )
                video_endpoint_mask = self._endpoint_valid_mask(video_mask, video_indices)
                if not bool(video_endpoint_mask):
                    raise ValueError(
                        f"Invalid video endpoint sample: trajectory={trajectory_name}, step={step}, "
                        f"indices=({int(video_indices[0])}, {int(video_indices[-1])})."
                    )
                return_dict["video_mask"] = video_mask
                return_dict["video_endpoint_mask"] = video_endpoint_mask
                return_dict["action_mask"] = action_mask
                if self.with_state:
                    state = []
                    for state_key in dataset.modality_keys["state"]:
                        state.append(data[state_key])
                    state = np.concatenate(state, axis=1).astype(np.float16)
                    state_mask, _ = self._valid_time_mask(
                        dataset,
                        trajectory_name,
                        dataset.modality_keys["state"][0],
                        step,
                    )
                    return_dict["state"] = state[0:1]
                    return_dict["state_0"] = state[0:1]
                    return_dict["state_T"] = state[-1:]
                    return_dict["state_mask"] = state_mask
