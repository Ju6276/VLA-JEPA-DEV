...
def _sample_video_delta_indices(endpoint_delta_indices: Sequence[int], num_frames: int) -> list[int]:
    """Endpoint-preserving video sampling over [t0, tT]."""
    if num_frames < 1:
        raise ValueError(f"`num_frames` must be >= 1, got {num_frames}.")
    endpoint_arr = np.asarray(endpoint_delta_indices, dtype=np.int64).reshape(-1)
    if endpoint_arr.size == 0:
        raise ValueError("`endpoint_delta_indices` cannot be empty.")
    if num_frames == 1:
        return [int(endpoint_arr[0])]
    if num_frames == 2:
        return [int(endpoint_arr[0]), int(endpoint_arr[-1])]
    sampled_pos = np.rint(np.linspace(0, endpoint_arr.size - 1, num=num_frames)).astype(np.int64)
    sampled_pos = np.clip(sampled_pos, 0, endpoint_arr.size - 1)
    sampled_pos[0] = 0
    sampled_pos[-1] = endpoint_arr.size - 1
    return endpoint_arr[sampled_pos].astype(np.int64).tolist()
...
def make_LeRobotSingleDataset(
    ...
    # H actions transition from state[0] to state[H]; video/state endpoints must include H.
    endpoint_indices = list(range(action_horizon + 1))
    observation_indices = _sample_video_delta_indices(endpoint_indices, video_horizon)
