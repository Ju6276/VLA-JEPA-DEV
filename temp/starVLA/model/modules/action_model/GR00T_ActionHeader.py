...
class FlowmatchingActionHead(nn.Module):
    ...
    def forward(
        ...
        loss_per_dim = (pred_actions - velocity) ** 2
        if actions_mask is not None:
            mask = actions_mask.to(device=loss_per_dim.device, dtype=loss_per_dim.dtype)
            while mask.dim() < loss_per_dim.dim():
                mask = mask.unsqueeze(-1)
            denom = mask.expand_as(loss_per_dim).sum().clamp_min(1.0)
            loss = (loss_per_dim * mask).sum() / denom
        else:
