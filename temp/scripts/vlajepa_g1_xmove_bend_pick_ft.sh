#!/bin/bash
# G1 X-move bend pick fine-tuning with V-JEPA 2.1 ViT-L/384 (weights under VJEPA21/).
# NOTE: DLC 默认使用 /bin/sh，不支持 source。请用 bash 运行本脚本：
#   bash scripts/vlajepa_g1_xmove_bend_pick_ft.sh
...
echo "Config: scripts/config/vlajepa_g1_xmove_bend_pick_ft.yaml (V-JEPA 2.1, 384px)"
echo "Encoder: VJEPA21/vjepa2_1_vitl_dist_vitG_384.pt"
echo "Dataset: dataset/simple/G1WholebodyXMoveBendPickTeleop-v0"
echo "Output: checkpoints/g1_xmove_bend_pick_vjepa21_ft"

accelerate launch \
  --config_file ./starVLA/config/deepseeds/deepspeed_zero2.yaml \
  --num_processes "${NUM_PROCESSES}" \
  ./starVLA/training/train_starvla.py \
  --config_yaml ./scripts/config/vlajepa_g1_xmove_bend_pick_ft.yaml
